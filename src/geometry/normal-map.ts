...
export function createUVMapData(
  width: number,
  height: number,
  radius: number,
): { data: Uint8Array; width: number; height: number } {
...
export function createUVMap(
  ...
  const result = createUVMapData(width, height, radius);
  return Texture.from({
    resource: result.data,
    width: result.width,
    height: result.height,
...
export function createEdgeMapData(
  width: number,
  height: number,
  radius: number,
  bevel: number,
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      let edgeValue = 0;
      if (bevel > 0 && dist >= 0 && dist < bevel) {
        edgeValue = ((bevel - dist) / bevel) * 255;
      }
...
export function createEdgeMap(
  ...
  const result = createEdgeMapData(width, height, radius, bevel);
  return Texture.from({
    resource: result.data,
    width: result.width,
    height: result.height,
...
export function createNormalMapData(
  width: number,
  height: number,
  radius: number,
  bevel: number,
...
export function createNormalMap(
  ...
  const result = createNormalMapData(width, height, radius, bevel, shape, invertNormals);
  return Texture.from({
    resource: result.data,
    width: result.width,
    height: result.height,
...
export function createAllMaps(
  ...
  shape: SurfaceShape = 'squircle',
  invertNormals: boolean = false,
  invertDisplacement: boolean = false,
): AllMapsResult {
  return {
    normalMap: createNormalMap(width, height, radius, bevel, shape, invertNormals),
    displacementMap: createDisplacementMap(width, height, radius, bevel, shape, invertDisplacement),
    uvMap: createUVMap(width, height, radius),
    edgeMap: createEdgeMap(width, height, radius, bevel),
  };
}
